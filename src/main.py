...
AUTO_ML_PROJECT = os.environ.get('AUTO_ML_PROJECT', '')
AUTO_ML_LOCATION = os.environ.get('AUTO_ML_LOCATION', '')
AUTO_ML_MODEL = os.environ.get('AUTO_ML_MODEL', '')
...
def classification(payload):
    ...
    project = AUTO_ML_PROJECT
    location = AUTO_ML_LOCATION
    model = payload.get('model_name', AUTO_ML_MODEL)
    ...
    try:
        ...
        name = prediction_client.model_path(project, location, AUTO_ML_MODEL)
        payload = {'image': {'image_bytes': imagebytes.getvalue()}}
        params = {'score_threshold': str(threshold)}
        logging.info('Calling AutoML {}'.format(name))
